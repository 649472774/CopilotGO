...
## 2026-09-07 — Phase 1 集成验收与 v0.2.0 版本准备

- 集成核心、原生 UI、Remote 与发布四条已批准的提交链，按补丁去重；没有合并重复的
  前置历史。集成验收基线为 `1fe639e`，当时保留 `0.1.33` / code 34。
- 修复完整 HTTP body 生命周期取消、登录/退出竞态、流式终止与错误、会话元数据回滚和
  删除复活。模型目录保留真实能力与失败状态，不用虚构列表覆盖已选模型。
- 旧会话图片迁移到内容寻址附件，保留原始恢复副本；既有标题、固定状态、时间、
  文本与中断回复保持兼容。凭据迁移使用真实 Android DataStore 与 Keystore 验收。
- 原生草稿在持久化接纳后才清除；发送、停止、删除确认、受限导入/导出、
  Markdown 选择复制和异步渲染保持独立可用。
- 实际设备验收发现并修复首次布局并发滚动、附件移除触控尺寸和短窗口确认框不可达。
  极短横屏的大字体输入改用同一编辑节点的横向重排与紧凑 Material 装饰；
  多行值不丢失，完整输入区域必须在可见视口内且位于系统栏之下。
- Remote 使用经过原有来源校验的离线 HTTPS 响应测试保留文档、崩溃恢复、代理、
  Cookie 回调、真实 IME 与系统 Back；没有放行不可信来源或强制伪造 READY。
- 自有集成构建执行 238 个 JVM 用例，零失败/错误/跳过，严格 lint 零错误。
  API 31 与 API 36 各执行全部 45 个原生用例（原 44 项加 5 次文档替换/重建回归），
  并补充真实 320dp、大字体、横屏、手势导航及输入区域完整可见性证据。
- 合成旧数据覆盖安装、迁移、两次重新打开与后续冷启动均保持完整；测试不使用真实
  账号、会话或用户原有 AVD。准备环境异常与早期失败记录保留，不被后续通过结果覆盖。
  一次旧测试销毁阶段的 SlotTable 异常未复现，保留未归因记录，不宣称依赖升级修复。
- 上述初始验收通过后，统一准备 `0.2.0` / code 35。包名仍为
  `com.tongxie.copilotgo.debug`，继续使用已记录的 v0.1.33 签名身份。
  签名包还需真实高版本 APK 校验与覆盖安装后的只读数据复核；主分支合并与稳定发布
  单独审批，不能把本条版本准备记录当成已发布证明。

此阶段不包含真正的 Web 搜索/MCP agent，也不宣称已完成真实 SSO 或完整语音
TalkBack 人工审计。Phase 2 仅在 Phase 1 接受后开展。
