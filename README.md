...
- 默认“自动”模式：天气、股价、新闻等问题在首次明确授权后执行真实联网搜索，
  普通知识问答不搜索。也可选择不使用工具的“普通聊天”，或手动开启完整 Agent。
- Agent 使用真实工具调用、审批与有界结果继续回答；工具记录和实际来源随会话保存。
  自动联网只开放内置公开网页工具，不能自动调用远程 MCP。
...
模型列表来自当前账户的服务端目录，不维护一份固定的“高级模型名单”。
按模型声明选择 Chat Completions 或 Responses，支持文字、视觉与相应的工具调用，
不会把 Responses-only 模型直接从列表中删除。账号/组织停用、未授权、不支持流式输出
或使用其他尚未接入协议的模型会显示不可用原因；服务端隐藏的内部别名仍不冒充可选模型。
新会话只从兼容模型中选择默认项，不会因展示诊断条目而选中一个被禁用的默认模型。

升级后可在模型选择器刷新目录。网站、不同 Copilot 产品与第三方客户端的开放范围可能
不同：订阅中出现某个模型，不等于该账户的当前 API 已开放它；应用不能绕过 GitHub
的组织策略、授权或配额。目录刷新失败时不使用旧缓存冒充已确认的可用权限。
...
在默认“自动”模式下，直接提问“今天北京天气如何”“今天微软股价多少”。
第一次会在聊天中说明搜索词、网址的对外发送范围；同意后才发送，取消则保留草稿和附件。
自动搜索授权可在联网工具设置中撤销；旧版本的对外发送同意不等于自动执行授权。
基础搜索使用 Exa 免密服务，不要求购买 API key；也可明确配置自己的 Exa key。

自动判断在本机完成，覆盖常见中英文时效问题、明确搜索请求和简短追问，不是通用语义分类器。
需要其他联网任务时可显式说“联网搜索……”，或手动开启 Agent。
当前信息请求会要求模型先搜索；搜索失败、没有可引用结果或模型没有执行工具都会明确报错，
不会降级为模型记忆并冒充实时数据。请求携带设备日期和时区，结果区分检索时间、
来源发布时间和行情时间；搜索摘录不保证是实时行情，仍需看交易时段与来源时间。

完整 Agent 中的 MCP 服务器需要先发现、选择工具，并使用支持工具调用的聊天模型。
默认逐次审批；明确开启的“自动允许公开网页读取”以及自动联网授权均不会授权任意 MCP，
服务器自称只读也不能跳过审批。模型或账户不支持工具调用时会要求手动选择兼容模型，
不会偷偷切换模型或清空草稿。
...
请求始终只使用单候选。Responses 的工具轮次在同一次运行内保留有界、不展示的
加密推理状态；这些不透明状态不写入会话文件。后续新一轮提问保留历史工具结果，
不重放缺少原协议状态的旧调用。完整的实际协议请求在接纳发送前以及每轮继续前检查预算。

免密搜索有额度和速率限制，失败会明确展示，不绕过挑战或编造结果。
针对 Exa 托管服务新增的 `objective` 必填参数，应用按实际发现的 Schema 发送固定的
来源质量与时效目标；不把完整对话或附件填进该参数，也不关闭参数校验来绕过错误。
